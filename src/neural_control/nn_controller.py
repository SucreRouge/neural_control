...
These wrenches are expressed in the '/base_link' frame.
...
- velocity and angular velocity are in body frame ('/base_link')
- wrenches and accelerations are in body frame ('/base_link')
...
class NN_controller:
	...
	def __init__(self, dof, kp, kd, kv, kw, N, sig, nn_limit,
		...
		self.prev_timestamp = 0  # previous call's timestamp
		...
		self.neuralwrench_pub = rospy.Publisher(neuralwrench_topic, WrenchStamped, queue_size=1)
...
########################
	...
	def get_wrench(self):
		...
		wrench.header.stamp = rospy.Time.from_sec(self.prev_timestamp)
		...
		neuralwrench.header.stamp = rospy.Time.from_sec(self.prev_timestamp)
...
########################
	...
	def give_new_state(self, pose_msg, twist_msg, timestamp_secs):
		"""
		...
		p, q, R, v, w, x = self.unpack(pose_msg.position, pose_msg.orientation,
									   twist_msg.linear, twist_msg.angular)
		self.p = p
		self.q = q
		self.R = R
		self.v = v
		...
		if self.prev_timestamp:
			self.dt = timestamp_secs - self.prev_timestamp
		self.prev_timestamp = timestamp_secs
...
########################
	...
	def give_new_reference(self, pose_msg, twist_msg):
		"""
